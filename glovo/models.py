...
class Order(models.Model):
    ...
    status_order = models.CharField(max_length=32, choices=STATUS_ORDER_CHOICES, default='ожидает обработки')
...
class ReviewCourier(models.Model):
    user_name = models.ForeignKey(UserProfile, on_delete=models.CASCADE)
    courier = models.ForeignKey(Courier, on_delete=models.CASCADE, related_name='courier_reviews', null=True, blank=True)
    text = models.TextField()
    stars = models.IntegerField(choices=[(i, str(i)) for i in range(1, 6)], verbose_name='рейтинг', null=True, blank=True)
    parent = models.ForeignKey('self', on_delete=models.SET_NULL, null=True, blank=True)
